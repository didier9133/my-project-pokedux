...
import { getPokemonsAll } from "./service/getPokemons";
import getURLPath from "./config";
import { SET_POKEMONS_NAMES } from "./features/dataSlice";

function App() {
  const dispatch = useDispatch();
  const loading = useSelector((state) => state.ui.loading);
  const pokemones = useSelector((state) => state.data.pokemons);
  const pokemonsSearched = useSelector((state) => state.data.pokemonsSearched);
  const ALLPOKEMONS = 100000;

  useEffect(() => {
    const setStateInitial = async () => {
      const URL = getURLPath({ offset: 0, limit: ALLPOKEMONS });
      const pokemonsAll = await getPokemonsAll(URL);

      dispatch(getPokemonsThunk({ url: BASE_URL }));
      dispatch(SET_POKEMONS_NAMES(pokemonsAll));
    };
  ...
  return (
    <>
      {loading ? (
        ...
        <div className="font-Poppins max-w-5xl m-auto overflow-x-hidden">
          <Header />
          <Pages />
          <PokemonList pokemones={pokemonsSearched || pokemones} />
          <Pages />
          <FooterComponent />
        </div>
      )}
